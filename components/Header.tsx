  ...
  return (
    <header className={`fixed top-0 inset-x-0 z-50 flex items-center justify-between px-8 py-6 transition-all duration-300 ${scrolled ? 'bg-black/80 backdrop-blur-xl border-b border-white/[0.06]' : ''}`}>
      ...
      <nav className="flex items-center gap-10">
        ...
        <Link
          href="/order"
          className={pathname === '/order' ? activeLink : navLink}
        >
          Order
        </Link>
      </nav>
          ...
          {profile ? (
            <motion.div key="user" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              >
                ...
                <span className="text-[11px] tracking-[0.2em] uppercase text-zinc-500 hover:text-white transition-colors hidden sm:block">
